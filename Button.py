import pygame as pg
class Button():
    def __init__(self, coords, dimensions, bg_color = None, border_color = (255, 255, 255), hover_bg_color = None, hover_border_color = (160, 160, 160), command = None, args = None):
        """
    ...
    def print_button(self, screen, mouse):
        if self.is_inside_button(mouse):
            if self.hover_border_color is not None:
                pg.draw.rect(screen, self.hover_bg_color, ((self.posx, self.posy), (self.width, self.height)), width = 3)
            else:
                pg.draw.rect(screen, self.hover_bg_color, ((self.posx, self.posy), (self.width, self.height)))
        else:
            if self.border_color is not None:
                pg.draw.rect(screen, self.bg_color, ((self.posx, self.posy), (self.width, self.height)), width = 3)
            else:
